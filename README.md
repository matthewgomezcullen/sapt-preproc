...
1. Run selected Configuration Interaction over the space MP2 capped, diagonalise its one-particle density, and keep the natural orbitals with $\text{lo} \le n_{i} \le \text{hi}$. An orbital above the window is doubly occupied and retires its pair to the core; one below it is empty and joins the virtuals.
...
- **Electronic-structure software:** the original used TeraChem/Lightspeed for classical SCF and integral generation, Gaussian for structural calculations, and in-house quantum code. This implementation substitutes PySCF throughout, including for the selected CI, which the original ran in Dice.
...
- **Selected CI:** PySCF's own selected CI stands in for Dice. It is variational and deterministic and runs in process, but has no semistochastic perturbative correction, and it does not reach fifty orbitals: on the fragment it costs $0.1$ s at eight orbitals, $15$ s at sixteen and $278$ s at twenty. Only the density is read downstream, never the energy.
