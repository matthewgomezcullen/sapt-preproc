...
- `src/sapt.py` — a `SAPT` stage class mirroring `SolveLigand`: reads $A$'s artefacts and the pose SCFs, writes one record per pose, resumes from what a previous run left. `elst`, `exch` and `interaction` fill a list per pose, and `exch` keeps the cumulant's share as `cumulants`. Step 6's `join`, `rank`, `summarise`, `FIELDS` and `SUMMARY_FIELDS` sit beside it.
