...
In two places: $D^{A}$, through every term of steps 3 and 4, and $\lambda_A$, through $E_{\text{exch}}[\lambda_A]$ alone. A determinant has no cumulant and an idempotent density, so a doubly occupied active space has to give back SAPT(RHF) exactly, whatever the window. That is the consistency test, and it runs through the same code as the correlated path. Over the water dimer the correlated density moves the exchange by $+1.0 \times 10^{-4}$ Hartree and the cumulant moves it by $-2.2 \times 10^{-5}$, both past the tables' last digit. A pose's record keeps $E_{\text{exch}}[\lambda_A]$ apart, so the cumulant's share is read without a second run. The density's share is the difference from a SAPT(RHF) run against the protein's own RHF density.

### 6. Reranking

`sapt.join` gives each row of `confidence.csv` its pose's `elst`, `exch` and `cumulant`, the share of `exch` the cumulant carries, keyed by complex and source as `confidence.join` keys its scores, and `interaction` = `elst` + `exch`. A pose without energies is dropped and reported. `sapt.rank` numbers each complex's poses from the lowest `interaction` as `rank_sapt`, a tie going to DiffDock's rank, and `sapt.summarise` adds `top1_sapt` to `confidence.summarise`'s row, so the three top-1s are over the same poses. They are written as `sapt.csv` and `sapt_summary.csv` beside `confidence.py`'s tables, through its `write_table` and `read_table`.
...
- `src/sapt.py` — a `SAPT` stage class mirroring `SolveLigand`: reads $A$'s artefacts and the pose SCFs, writes one record per pose, resumes from what a previous run left. Step 6's `join`, `rank`, `summarise`, `FIELDS` and `SUMMARY_FIELDS` sit beside it.
- `src/confidence.py` — `read_table` learns the new columns: `rank_sapt` an integer, the four energies decimals, `top1_sapt` a boolean.
- `src/run.py` — a fourth stage after `CASCI`, skipped when its artefact is kept.
...
**Cost.** Per pose, eight J/K builds for the separable part, each over one block of the dimer's functions, and two J builds of $N_{\text{act}}(N_{\text{act}}+1)/2 = 105$ pair densities for the cumulant. The largest class is $(AA|AB)$, about $n_A^{3} n_B / 2 \approx 3 \times 10^{11}$ integrals a pass for the 1695-function cutout, and the separable part's two builds over it can share one `get_jk` call. `get_jk` screens nothing unless handed a `vhfopt`, and most of those integrals vanish away from the interface, so screening is the first thing to try if the cluster run is slow. Twelve complexes, 430 poses.

**TBC.** Whether D3 is in scope; artefact naming, and whether a pose's record carries more than `elst`, `exch` and `cumulant`; whether the driver refuses a complex whose protein never reached CASCI or falls back to SAPT(RHF), and whether it runs SAPT(RHF) beside SAPT(CASCI) to give the density's share per pose.
