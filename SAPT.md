...
- `src/utils/sapt.py` — steps 1--5: `density` and `active`; the integral layer, `dimer`, `overlap`, `potential`, `attraction`, `repulsion`, `coulomb`, `generalized` and `generalized_coulomb`; `electrostatics`, `exchange`, `cumulant` and `cumulant_exchange`, and `exchange_parts`, which gives both parts of the exchange from one pass a block.
- `src/sapt.py` — a `SAPT` stage class mirroring `EncodeProtein`: reads $A$'s artefacts and the pose SCFs, and `interaction` scores the poses one at a time into a list per term, `exchanges` carrying the cumulant's share and `cumulants` keeping it apart. Each pose is kept in one record per complex, `<complex>_sapt.npz`, beside its `source`, as soon as it is scored, so a stage handed that directory again scores only the poses it lacks, and a task stopped at the time limit resumes after the last pose it kept. Step 6's `join`, `rank`, `summarise`, `FIELDS` and `SUMMARY_FIELDS` sit beside it, and `run`, the script's entry point.
...
**Cost.** Per pose, the separable part's eight J/K contractions take four `get_jk` calls, one a block of the dimer's functions, and the cumulant's $N_{\text{act}}(N_{\text{act}}+1)/2 = 105$ pair densities ride along the calls over $(AA|AB)$ and $(AA|BB)$. The largest class is $(AA|AB)$, about $n_A^{3} n_B / 2 \approx 3 \times 10^{11}$ integrals a pass for the 1695-function cutout, so a pose passes over it once. On a 12-thread laptop that pass takes about 8 minutes with one density and 24 with the cumulant's, most of a pose's cost, and the largest cutouts cost up to 23 times as much a pose, which is why each pose is kept as it is scored. A pair over one monomer's functions keeps its permutational symmetry wherever `get_jk` has a kernel for the contraction, which halves that pass. `get_jk` screens nothing unless handed a `vhfopt`, and most of those integrals vanish away from the interface, so screening is the first thing to try if the cluster run is slow. The cumulant's 105 pair densities are held at once over $A$'s functions, about 2.4 GB for that cutout. Twelve complexes, 430 poses.

**Skipped cumulants.** `sapt.WITHOUT_CUMULANT` names 7XFA_D9J and 7PRM_81I, the two complexes whose $(AA|AB)$ pass segfaulted with the cumulant's pair densities riding on it: its thread buffers, `threads` $\times$ `densities` $\times n_A n_B$ doubles, reached 43 and 47 GB where 33 GB ran, so $2^{32}$ doubles is the line and PySCF's C driver appears to overflow a 32-bit size beyond it. Chunking that pass costs about 150 hours a complex, so both are scored over their correlated densities alone: they keep the larger share of the correlation, the density's, and none of the cumulant's, and their `cumulant` column is zero. Temporary, and to be removed once the pass is chunked.
