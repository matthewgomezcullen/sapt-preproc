...
def lowest(hamiltonian, nelecas):
    ...
    Counting set bits is agnostic to whether the mapping blocks the spins or interleaves them, so
        this does not depend on a convention the test should not know about.
    """
...
def test_a_rebuilt_molecule_gives_the_same_integrals():
    ...
    This is what the stored geometry buys, so it is asserted rather than assumed.
    """
...
# --------------------------------------------------------------------------------------------
# The qubit Hamiltonian, against exact diagonalisation of the same space.
# --------------------------------------------------------------------------------------------
...
def test_the_qubit_hamiltonian_holds_the_energy_casci_finds():
    ...
    One assertion covering the index order of the two-electron integrals, the doubling into spin
        orbitals, the core energy, the half in front of the two-electron term, and the mapping.
    """
    encoded = capped()
...
def test_another_mapping_is_another_operator():
    """
    The argument reaches the library rather than being ignored.
