...
def molecule():
    """
    A real molecule, because `save` stores it and takes the digest from it. Four orbitals.
    """
...
def over_the_fragment():
    """
    A finished run over the fragment's own molecule, for the tests that reprepare it.

    Building the molecule needs no SCF, so this stays cheap.
    """
...
def strip(path, *drop, **replace):
    """
    Rewrite a stored result without some of its keys, standing in for a file an older run wrote.
    """
...
# --------------------------------------------------------------------------------------------
# What makes a stored run self-sufficient.
# --------------------------------------------------------------------------------------------


def test_the_molecule_is_stored(tmp_path):
    """
    The geometry, basis, charge, spin and atom ordering, so nothing has to be prepared again.
    """
    written = finished()
    path = run.path(FRAGMENT, str(tmp_path))
...
def test_the_stored_molecule_rebuilds(tmp_path):
    """
    What comes back is the molecule that went in, not merely a string.
    """
    written = finished()
    path = run.path(FRAGMENT, str(tmp_path))
    run.save(written, FRAGMENT, path)
...
def test_the_checkpoint_digest_is_stored(tmp_path):
    """
    Ties the result to the SCF it came from, so a resume can tell if the geometry has drifted.
    """
    written = finished()
    path = run.path(FRAGMENT, str(tmp_path))
...
def test_the_digest_survives_a_rebuild():
    """
    A molecule through dumps and loads keeps its digest, so it still finds its own checkpoint.
    """
    mol = molecule()
...
def test_a_stored_run_resumes_without_preparing_anything(tmp_path):
    """
    A result holding its molecule needs no protein, no poses and no SCF.
    """
    out = str(tmp_path)
    written = finished()
...
def test_a_resumed_run_reaches_its_hamiltonian(tmp_path):
    """
    The point of storing the molecule.
    """
    out = str(tmp_path)
    run.save(finished(), FRAGMENT, run.path(FRAGMENT, out))
...
def test_a_stored_run_without_a_molecule_is_prepared_again(tmp_path):
    """
    The three complexes solved before the molecule was stored. Bridge, not design.
    """
    out = str(tmp_path)
...
def test_a_geometry_that_no_longer_matches_is_refused(tmp_path):
    """
    Repreparing is only reproducible while the seed and the reference platform hold. A drifted
        geometry makes the stored orbitals meaningless, and nothing else would raise.
    """
    out = str(tmp_path)
    path = run.path(FRAGMENT, out)
    run.save(over_the_fragment(), FRAGMENT, path)
